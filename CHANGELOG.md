# Changelog

All notable changes to EasyMapper are documented in this file. The project follows
[Semantic Versioning](https://semver.org/).

## [Unreleased]

### Added

- Convention-first mapping without mandatory profiles or startup registration.
- Strongly typed `Only`, `Except`, and `Bind(...).From(...)` rules.
- Cached expression-tree execution plans.
- Case matching, null assignment, and scalar conversion policies.
- Unit, smoke, package-consumer, and sustained-load verification.

## [0.1.0-alpha.1] - Unreleased

- First preview of the redesigned EasyMapper architecture.
